...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/GainproxParams"
else
    PARAMS_DIR="$HOME/.gainprox-params"
fi
...
DOWNLOAD_URL="https://github.com/gprotocolx/gainprox/releases/tag/1.2.15.3"
...
# fetch methods can be disabled with GPRX_DISABLE_SOMETHING=1
GPRX_DISABLE_WGET="${GPRX_DISABLE_WGET:-}"
#GPRX_DISABLE_IPFS="${GPRX_DISABLE_IPFS:-}"
GPRX_DISABLE_CURL="${GPRX_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$GPRX_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFSCMD" ] || ! [ -z "$GPRX_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$GPRX_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the Gainprox parameters!
...
main() {
    ...
    cat <<EOF
GPRX - fetch-params.sh

This script will fetch the Gainprox SNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Gainprox SNARK parameters. Note that it is
