...
PACKAGE_VERSION=$($SRC_PATH/src/gainproxd --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/gainproxd $DEB_BIN
cp $SRC_PATH/src/gainprox-cli $DEB_BIN
cp $SRC_PATH/util/fetch-params.sh $DEB_BIN/gainprox-fetch-params
# Copy docs
...
# Copy manpages
cp $SRC_DOC/man/gainproxd.1 $DEB_MAN
cp $SRC_DOC/man/gainprox-cli.1 $DEB_MAN
cp $SRC_DOC/man/gainprox-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/gainproxd.bash-completion $DEB_CMP/gainproxd
cp $SRC_PATH/contrib/gainprox-cli.bash-completion $DEB_CMP/gainprox-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/gainproxd.1
gzip --best -n $DEB_MAN/gainprox-cli.1
gzip --best -n $DEB_MAN/gainprox-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/gainproxd $DEB_BIN/gainprox-cli
