...
const char * const BITCOIN_CONF_FILENAME = "gainprox.conf";
const char * const BITCOIN_PID_FILENAME = "gainproxd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "gainproxd -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\GainproxCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\GainproxCore
    // Mac: ~/Library/Application Support/GainproxCore
    // Unix: ~/.gainproxcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "GainproxCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/GainproxCore";
#else
    // Unix
    return pathRet / ".gainproxcore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty gainprox.conf if it does not excist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Gainprox Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2021, nEndYear) + "The Gainprox Core developers";
    }
