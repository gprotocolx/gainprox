...
# gainprox_hash
RUN git clone https://github.com/gprotocolx/gainprox_hash
RUN cd gainprox_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} gainprox
RUN useradd -u ${USER_ID} -g gainprox -s /bin/bash -m -d /gainprox gainprox
...
RUN mkdir /gainprox-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /gainprox-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /gainprox-src
